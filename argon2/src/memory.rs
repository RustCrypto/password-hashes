...
impl<'a> SegmentViews<'a> for &'a mut [Block] {
    fn segment_views(&mut self, slice: usize, lanes: usize) -> SegmentViewIter<'_> {
        // SAFETY: a pointer to the buffer or a `&mut [Block]` can never be null.
        let blocks = unsafe { NonNull::new_unchecked(self.as_mut_ptr()) };
        ...
        unsafe { SegmentViewIter::new(blocks, self.len(), slice, lanes) }
    }
}
